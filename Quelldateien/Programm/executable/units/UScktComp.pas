...
unit UScktComp;
{
  ...
  year:       2008/2009
  desc:       This file is a copy of ScktComp.pas from Borland Software
              Corporation. The only differents from the original file are a few
              changes on the TCustomWinSocket class.
  ...
  license:    creative commons by-nc-sa 3.0 germany
   ...
   this licence effects only the changes I made.
}
...
//------------------------------------------------------------------------------
//-- eigene Deklaration --------------------------------------------------------
    function RL_SendText: integer;
    procedure RL_AddText(text:string);
    property RL_username: string read RL_var_username write RL_var_username;
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//-- eigene Methode ------------------------------------------------------------
procedure TCustomWinSocket.RL_AddText(text: string);
begin
{
  RL_AddText('');
  -> RL_Text = '';
  ...
  RL_AddText('befehl2');
  RL_AddText('befehl3');
  -> RL_Text = 'befehl1#befehl2#befehl3#'
  -> n = 3
}
 if text <> '' then
  begin
   self.RL_var_text := self.RL_var_text + text + '#';  // neuen Befehl anhängen
   self.RL_var_n:=self.RL_var_n+1;           // Zähler der Befehle um 1 erhöhen
  end;
end;
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
//-- eigene Methode ------------------------------------------------------------
function TCustomWinSocket.RL_SendText: integer;
begin
{
  RL_SendText;
  -> versendet: nichts
  ...
  RL_AddText(befehl3);
  RL_SendText;
  -> versendet: '3#befehl1#befehl2#befehl3#'
}
 result:=0;
 if self.RL_var_n > 0 then
  begin
   result:=self.SendText(inttostr(self.RL_var_n)+'#'+self.RL_var_text);
   self.RL_var_text:=''; //leeren
   self.RL_var_n:=0;     //leeren
  end;
end;
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
